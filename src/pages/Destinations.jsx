  ...
  if (view === 'detail' && selectedPlace) {
    return (
      <div id="destination-detail-view" className="fade-in" style={{ paddingTop: '76px' }}>
        <div className="detail-hero" id="detailHeroBg" style={{ backgroundImage: `linear-gradient(to bottom, rgba(0, 119, 182, 0.4), var(--bg-dark)), url('${selectedPlace.img}')`, backgroundSize: 'cover', backgroundPosition: 'center', padding: '100px 0 50px' }}>
            <div className="container pt-5 pb-5">
                <button className="back-link mb-3 d-inline-block border-0 bg-transparent text-white text-decoration-underline shadow-sm" onClick={handleCloseDetail} style={{cursor: 'pointer'}}>← Back to All Places</button>
                <h1 className="hero-title mb-3" id="detailTitle">{selectedPlace.name}</h1>
                <div className="d-flex gap-3 align-items-center">
                    <span className="badge rounded-pill text-white" style={{ backgroundColor: 'var(--accent-color)' }} id="detailType">{selectedPlace.type}</span>
                    <span className="text-white font-montserrat opacity-75"><i className="fa-solid fa-location-dot me-1"></i> <span id="detailRegion">{selectedPlace.region}</span></span>
                </div>
            </div>
        </div>
        <div className="container py-5">
            <div className="row g-5">
                <div className="col-lg-8">
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box teal-hover-box shadow-sm border border-primary border-opacity-10">
                    ...
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box teal-hover-box shadow-sm border border-primary border-opacity-10">
                        ...
                        <ul className="list-unstyled text-grey m-0">
                            {selectedPlace.bed && <li className="mb-2"><strong className="text-navy">Bed Type:</strong> {selectedPlace.bed.join(', ')}</li>}
                            {selectedPlace.bedrooms && <li className="mb-2"><strong className="text-navy">Bedrooms:</strong> {selectedPlace.bedrooms.join(', ')}</li>}
                            {selectedPlace.beachAccess && <li className="mb-2"><strong className="text-navy">Beach Access:</strong> {selectedPlace.beachAccess}</li>}
                            {selectedPlace.travelStyle && selectedPlace.travelStyle.includes('Hiking/Mountaineering') && <li className="mb-2"><strong className="text-navy">Hiking:</strong> Yes</li>}
                            {selectedPlace.distance && <li className="mb-2"><strong className="text-navy">Distance:</strong> {selectedPlace.distance}</li>}
                            {selectedPlace.family && <li className="mb-2"><strong className="text-navy">Family:</strong> {selectedPlace.family}</li>}
                        </ul>
                    </div>
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box teal-hover-box shadow-sm border border-primary border-opacity-10">
                        <h6 className="text-accent fw-bold mb-3 font-montserrat">360° VIEW</h6>
                        <div id="panorama" className="rounded-3 border border-primary border-opacity-25 shadow-sm" style={{ width: '100%', height: '400px', backgroundColor: 'var(--bg-dark)' }}></div>
                    </div>
                </div>

                <div className="col-lg-4">
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box text-center teal-hover-box shadow-sm border border-primary border-opacity-10">
                    ...
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box text-center teal-hover-box shadow-sm border border-primary border-opacity-10">
                        <div className="d-flex align-items-center justify-content-center gap-2 mb-3">
                            ...
                            <button className="btn btn-sm btn-link p-0 ms-2 text-primary" onClick={handleRefreshWeather} title="Refresh Weather"><i className={`fa-solid fa-rotate-right ${isRefreshing ? 'fa-spin' : ''}`}></i></button>
                        </div>
                        <h1 className="fw-bold mb-1 text-accent" style={{ fontSize: '3.5rem' }} id="weatherTemp">{weather}</h1>
                    </div>
    ...
    <div id="destinations-main-view" className="fade-in">
        <section className="destinations-hero">
            <div className="container text-center mb-4 scroll-reveal visible">
                ...
                <p className="section-desc mb-0 text-white">Browse the best accommodations in the Philippines</p>
            </div>
            ...
            <div className="container">
                <div className="row">
                    <div className="col-lg-4 col-xl-3 mb-4 scroll-reveal visible">
                        <div className="d-lg-none mb-3"><button className="btn w-100 d-flex justify-content-between align-items-center p-3 rounded-3 border border-primary border-opacity-25 shadow-sm text-navy" type="button" data-bs-toggle="collapse" data-bs-target="#filterSidebar" style={{ backgroundColor: 'var(--card-bg)' }}><span className="font-montserrat fw-bold"><i className="fa-solid fa-sliders text-accent me-2"></i> Filters {activeCheckboxes.length > 0 ? `(${activeCheckboxes.length})` : ''}</span><i className="fa-solid fa-chevron-down"></i></button></div>
                        <div className="collapse d-lg-block" id="filterSidebar">
                            <div className="sidebar-filter-container bg-card-dark p-4 rounded-4 border border-primary border-opacity-25 sticky-top" style={{ top: '100px', maxHeight: '80vh', overflowY: 'auto' }}>
                                <div className="d-flex justify-content-between align-items-center mb-4 border-bottom border-primary border-opacity-25 pb-3"><h5 className="text-navy fw-bold font-montserrat m-0 d-none d-lg-block"><i className="fa-solid fa-sliders text-accent me-2"></i> Filters</h5>{activeCheckboxes.length > 0 && (<button className="btn btn-sm btn-link text-grey p-0 text-decoration-none" onClick={() => setActiveCheckboxes([])}>Clear All</button>)}</div>
                                {filterCategories.map(category => (
                                    <div className="filter-section mb-4" key={category.title}>
                                        <h6 className="filter-title fw-bold text-navy mb-3" style={{ fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px' }}>{category.title}</h6>
                                        {category.options.map(option => {
                                            const count = getCount(option); const isActive = activeCheckboxes.includes(option);
                                            if (count === 0 && !isActive) return null; 
                                            return (
                                                <div className="form-check mb-2" key={option}><input className="form-check-input filter-checkbox" type="checkbox" onChange={() => handleToggleCheckbox(option)} checked={isActive} id={`filter-${option.replace(/[^a-zA-Z0-9]/g, '-')}`} style={{ cursor: 'pointer', accentColor: 'var(--accent-color)' }}/><label className="form-check-label w-100 d-flex justify-content-between align-items-center text-grey" htmlFor={`filter-${option.replace(/[^a-zA-Z0-9]/g, '-')}`} style={{ cursor: 'pointer', fontSize: '0.85rem' }}><span className={isActive ? 'text-accent fw-bold' : ''}>{option}</span><span className="badge bg-primary bg-opacity-10 rounded-pill text-navy">{count}</span></label></div>
                                            );
                                        })}
                                    </div>
                                ))}
                    ...
                    <div className="col-lg-8 col-xl-9">
                        {isDefaultView ? (
                            <div className="fade-in">
                                <div className="d-flex justify-content-between align-items-center mb-3"><h4 className="text-navy font-montserrat fw-bold mb-0">Explore Regions</h4><span className="text-grey small fw-bold">6 Major Regions</span></div>
                                <div className="fanned-stack-container mt-4" style={{ height: '500px', position: 'relative' }}>
                                    <button className="stack-nav-btn prev-btn shadow" onClick={() => rotateRegionStack('prev')}><i className="fa-solid fa-chevron-left"></i></button><button className="stack-nav-btn next-btn shadow" onClick={() => rotateRegionStack('next')}><i className="fa-solid fa-chevron-right"></i></button>
                                    {regions.map((region, index) => (
                                        <div key={region.id} className={`fanned-card-wrapper ${regionPositions[index]}`} onClick={() => { if(regionPositions[index] === 'pos-center') { const valueMap = { 'Manila': 'Manila', 'Cebu': 'Cebu', 'Palawan': 'Palawan', 'Bohol': 'Bohol', 'Boracay': 'Boracay', 'Banaue': 'Ifugao' }; setSelectedRegion(valueMap[region.id] || region.id); window.scrollTo({ top: 400, behavior: 'smooth' }); } else if(regionPositions[index].includes('right')) { rotateRegionStack('next'); } else { rotateRegionStack('prev'); } }}>
                                            <div className="card h-100 border-0"><div className="card-img-wrapper"><span className="card-badge">{region.typeBadge}</span><img src={region.image} className="card-img-top" alt={region.name} /></div><div className="card-body"><div className="card-location"><i className="fa-solid fa-location-dot"></i> {region.locationLabel}</div><h5 className="card-title text-navy">{region.name}</h5><p className="card-text text-grey">{region.desc}</p><div className="mt-auto pt-3 border-top border-primary border-opacity-10 text-center"><span className="btn-text-link m-0" style={{ fontSize: '0.8rem' }}>View Accommodations <i className="fa-solid fa-arrow-right"></i></span></div></div></div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="fade-in">
                                <div className="d-flex justify-content-between align-items-center mb-3"><h4 className="text-navy font-montserrat fw-bold mb-0">{selectedRegion === 'All' ? 'Search Results' : (selectedRegion === 'Boracay' ? 'Aklan (Boracay)' : (selectedRegion === 'Ifugao' ? 'Ifugao (Banaue)' : selectedRegion))}</h4><span className="text-grey small fw-bold">{filteredPlaces.length} destination{filteredPlaces.length !== 1 ? 's' : ''} found</span></div>
                                <div className="row g-4">
                                    {filteredPlaces.length === 0 ? (
                                        <div className="col-12 text-center text-grey py-5 border border-primary border-opacity-25 border-dashed rounded-4"><i className="fa-solid fa-filter-circle-xmark fs-1 mb-3 opacity-50 text-primary"></i><h5 className="text-navy fw-bold">No matching accommodations</h5><p className="small mb-0">Try removing some of your active filters from the sidebar.</p></div>
                                    ) : (
                                        filteredPlaces.map((place) => (
                                            <div key={place.id} className="col-md-6 scroll-reveal visible">
                                                <div className="card h-100 border-0 shadow-sm teal-hover-box">
                                                    ...
                                                    <div className="card-body">
                                                        <div className="card-location"><i className="fa-solid fa-location-dot"></i> {place.region}</div><h5 className="card-title text-navy">{place.name}</h5><p className="card-text text-grey small mb-3">{place.type} • {place.distance || 'Various'}</p>
                                                        <div className="d-flex flex-wrap gap-2 mb-3">{(place.facilities || []).slice(0, 3).map(fac => (<span key={fac} className="badge bg-primary bg-opacity-10 text-navy fw-normal">{fac}</span>))}</div>
                                                        <div className="mt-auto pt-3 border-top border-primary border-opacity-10">
                                                            <button className="explore-link border-0 bg-transparent p-0 text-start w-100 d-flex justify-content-between align-items-center" onClick={() => handleOpenDetail(place)}><span>{t('view_details', 'View Details')}</span> <i className="fa-solid fa-arrow-right text-accent"></i></button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
