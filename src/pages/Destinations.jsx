...
const Destinations = () => {
  ...
  // --- FILTER ARRAYS ---
  const paymentOptions = ['Free cancellation', 'Pay at the hotel', 'Book now, pay later', 'Pay now', 'Book without credit card'];
  const roomOffers = ['Breakfast included', 'Airport transfer', 'Exercise bike', 'Outside food delivery allowed', 'Treadmill', 'Dinner included', 'Lunch included', 'Early check-in', 'Vegetarian', 'Espresso machine with pods', 'Car rental', 'Gluten-free', 'Vegan', 'Dumbbells', 'Bottle of wine', 'Delivery from nearby convenience store', 'Recreation area access'];
  const facilitiesList = ['Swimming pool', 'Internet', 'Car park', 'Gym/fitness', 'Front desk [24-hour]', 'Family/child friendly', 'Non-smoking', 'Spa/sauna', 'Restaurants', 'Smoking area', 'Pets allowed', 'Nightclub', 'Facilities for disabled guests', 'Business facilities', 'Golf course [on-site]'];

  // --- DYNAMIC ACCOMMODATION DROPDOWN ---
  const hotelsForDropdown = selectedRegion === 'All' 
    ? allPlaces 
    : allPlaces.filter(p => p.region === selectedRegion);

  // Determine if we should show the Fanned Stack or the Grid
  const isDefaultView = selectedRegion === 'All' && selectedHotel === '' && searchKeyword === '' && activeCheckboxes.length === 0;

  // --- CATCH INCOMING SEARCH PARAMETERS FROM HOMEPAGE ---
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const searchParam = params.get('search');
    
    if (searchParam) {
      setSearchKeyword(searchParam); // Sets "camping"
      setSelectedRegion('All'); // Makes sure it searches everywhere
    }
  }, [location.search]);

  // --- FANNED STACK ROTATION ---
  ...
  useEffect(() => {
    ...
    if (selectedHotel !== '') result = result.filter(p => p.id === selectedHotel);
    ...
    if (searchKeyword) {
      ...
      result = result.filter(p => 
        ...
        p.region.toLowerCase().includes(lowerKw) ||
        (p.facilities && p.facilities.some(f => f.toLowerCase().includes(lowerKw)))
      );
    }
    ...
    if (activeCheckboxes.length > 0) {
      result = result.filter(place => {
        const placeAttribs = [
          ...(place.payment || []), ...(place.roomOffers || []), ...(place.facilities || []), 
          place.hiking === 'Yes' ? 'Hiking/Mountaineering' : ''
        ];
  ...
  }, [selectedRegion, selectedHotel, searchKeyword, activeCheckboxes]);

  // --- PANNELLUM 360 VIEWER LOGIC ---
  useEffect(() => {
    if (view === 'detail' && selectedPlace && selectedPlace.panorama) {
      if (window.myPannellumViewer) {
        window.myPannellumViewer.destroy();
      }
      setTimeout(() => {
        if (document.getElementById('panorama') && window.pannellum) {
          window.myPannellumViewer = window.pannellum.viewer('panorama', {
            "type": "equirectangular",
            "panorama": selectedPlace.panorama,
            "autoLoad": true,
            "compass": true,
  ...
  const handleToggleCheckbox = (val) => {
    setActiveCheckboxes(prev => prev.includes(val) ? prev.filter(item => item !== val) : [...prev, val]);
  };

  const fetchWeather = async (lat, lon) => {
    const API_KEY = '423c732b34abb2959bd3dd661246d90f'; 
    try {
  ...
  const handleRefreshWeather = async () => {
    if (selectedPlace && selectedPlace.lat) {
      ...
      await fetchWeather(selectedPlace.lat, selectedPlace.lon);
  ...
  const handleOpenDetail = (place) => {
    ...
    setView('detail');
    if (place.lat && place.lon) fetchWeather(place.lat, place.lon);
    window.scrollTo(0, 0);
  };
      ...
      <div id="destination-detail-view" className="fade-in" style={{ paddingTop: '76px' }}>
        <div className="detail-hero" id="detailHeroBg" style={{ backgroundImage: `linear-gradient(to bottom, rgba(2, 26, 46, 0.4), var(--bg-dark)), url('${selectedPlace.img}')`, backgroundSize: 'cover', backgroundPosition: 'center', padding: '100px 0 50px' }}>
            <div className="container pt-5 pb-5">
                <button className="back-link mb-3 d-inline-block border-0 bg-transparent text-white text-decoration-underline" onClick={handleCloseDetail} style={{cursor: 'pointer'}}>
                  ← Back to All Places
                </button>
                ...
                <div className="col-lg-8">
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box teal-hover-box">
                        ...
                        <ul className="row list-unstyled text-white-50 m-0">
                            {[...(selectedPlace.payment || []), ...(selectedPlace.roomOffers || [])].map((item, idx) => (
                                <li key={idx} className="col-md-6 mb-2"><i className="fa-solid fa-check text-accent me-2"></i>{item}</li>
                            ))}
                        </ul>
                    </div>
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box teal-hover-box">
                        ...
                        <ul className="row list-unstyled text-white-50 m-0">
                            {(selectedPlace.facilities || []).map((fac, idx) => (
                                <li key={idx} className="col-md-6 mb-2"><i className="fa-solid fa-check text-accent me-2"></i>{fac}</li>
                            ))}
                        </ul>
                    </div>
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box teal-hover-box">
                        ...
                        <ul className="list-unstyled text-white-50 m-0">
                            ...
                            {selectedPlace.access && <li className="mb-2"><strong>Access:</strong> {selectedPlace.access}</li>}
                            {selectedPlace.hiking && <li className="mb-2"><strong>Hiking:</strong> {selectedPlace.hiking}</li>}
                ...
                <div className="col-lg-4">
                    <div className="bg-card-dark p-4 rounded-4 mb-4 detail-box text-center teal-hover-box">
                        <div className="d-flex align-items-center justify-content-center gap-2 mb-3">
                            ...
                            <button className="btn btn-sm btn-link p-0 ms-2 text-white-50" onClick={handleRefreshWeather} title="Refresh Weather">
                    ...
                    <a href={`/booking?package=${encodeURIComponent(selectedPlace.name)}`} className="btn btn-proceed w-100 py-3 text-uppercase font-montserrat fw-bold mb-3 text-decoration-none text-center d-block">Book Now</a>
                </div>
            </div>
        </div>
      </div>
    ...
    <div id="destinations-main-view" className="fade-in">
        <section className="destinations-hero">
            <div className="container text-center mb-4 scroll-reveal visible">
                <h1 className="hero-title" style={{ fontSize: '4rem' }}>FIND YOUR PLACE</h1>
                        ...
                        <div className="col-md-4">
                            ...
                            <div className="input-with-icon">
                                ...
                                <select className="form-control-dark w-100" value={selectedRegion} onChange={(e) => { setSelectedRegion(e.target.value); setSelectedHotel(''); }}>
                                    ...
                                    <option value="Boracay">Aklan (Boracay)</option>
        ...
        <section id="destinations" className="fade-in mb-5" style={{ marginTop: '40px' }}>
            <div className="container">
                <div className="row">
                    ...
                    {/* SIDEBAR FILTERS */}
                    <div className="col-lg-3 mb-4 scroll-reveal visible">
                        <div className="sidebar-filter-container">
                            <div className="filter-section">
                                <h6 className="filter-title">Payment Options</h6>
                                {paymentOptions.map(option => (
                                    <div className="form-check" key={option}>
                                        <input className="form-check-input filter-checkbox" type="checkbox" onChange={() => handleToggleCheckbox(option)} checked={activeCheckboxes.includes(option)} />
                                        <label className="form-check-label">{option}</label>
                                    </div>
                                ))}
                            </div>
                            <div className="filter-section">
                                <h6 className="filter-title">Room Offers</h6>
                                {roomOffers.map(offer => (
                                    <div className="form-check" key={offer}>
                                        <input className="form-check-input filter-checkbox" type="checkbox" onChange={() => handleToggleCheckbox(offer)} checked={activeCheckboxes.includes(offer)} />
                                        <label className="form-check-label">{offer}</label>
                                    </div>
                                ))}
                            </div>
                             <div className="filter-section">
                                <h6 className="filter-title">Facilities</h6>
                                {facilitiesList.map(facility => (
                                    <div className="form-check" key={facility}>
                                        <input className="form-check-input filter-checkbox" type="checkbox" onChange={() => handleToggleCheckbox(facility)} checked={activeCheckboxes.includes(facility)} />
                        ...
                        {isDefaultView ? (
                            <div className="fade-in">
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <span className="text-white-50 small">6 Major Regions</span>
                                </div>
                                
                                <div className="fanned-stack-container mt-4" style={{ height: '500px', position: 'relative' }}>
                                    <button className="stack-nav-btn prev-btn" onClick={() => rotateRegionStack('prev')}><i className="fa-solid fa-chevron-left"></i></button>
                                    <button className="stack-nav-btn next-btn" onClick={() => rotateRegionStack('next')}><i className="fa-solid fa-chevron-right"></i></button>
                                    
                                    {regions.map((region, index) => (
                                        <div 
                                            key={region.id} 
                                            className={`fanned-card-wrapper ${regionPositions[index]}`}
                                            onClick={() => {
                                                if(regionPositions[index] === 'pos-center') {
                                                    const valueMap = { 'Manila': 'Manila', 'Cebu': 'Cebu', 'Palawan': 'Palawan', 'Bohol': 'Bohol', 'Boracay': 'Boracay', 'Banaue': 'Ifugao' };
                                                    setSelectedRegion(valueMap[region.id] || region.id);
                                                    window.scrollTo({ top: 400, behavior: 'smooth' }); 
                                                } else if(regionPositions[index].includes('right')) {
                                                    rotateRegionStack('next');
                                                } else {
                                                    rotateRegionStack('prev');
                                                }
                                            }}
                                        >
                                            <div className="card h-100">
                                                <div className="card-img-wrapper">
                                                    <span className="card-badge">{region.typeBadge}</span>
                                                ...
                                                <div className="card-body">
                                                    ...
                                                    <p className="card-text">{region.desc}</p>
                                                    <div className="mt-auto pt-3 border-top border-secondary border-opacity-25 text-center">
                                                        <span className="btn-text-link m-0" style={{ fontSize: '0.8rem' }}>View Accommodations <i className="fa-solid fa-arrow-right"></i></span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="fade-in">
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                    <h4 className="text-white font-montserrat fw-bold mb-0">
                                        {selectedRegion === 'All' ? 'Search Results' : (selectedRegion === 'Boracay' ? 'Aklan (Boracay)' : (selectedRegion === 'Ifugao' ? 'Ifugao (Banaue)' : selectedRegion))}
                                    </h4>
                                    <span className="text-white-50 small">{filteredPlaces.length} destination{filteredPlaces.length !== 1 ? 's' : ''} found</span>
                                </div>
                                
                                <div className="row g-4">
                                    {filteredPlaces.length === 0 ? (
                                        <div className="col-12 text-center text-white-50 py-5">No places match your criteria.</div>
                                    ) : (
                                            ...
                                            <div key={place.id} className="col-md-6 scroll-reveal visible">
                                                <div className="card h-100 teal-hover-box" style={{ border: '1px solid rgba(255,255,255,0.1)' }}>
                                                    ...
                                                    <div className="card-body">
                                                        ...
                                                        <button className="explore-link border-0 bg-transparent p-0 text-start" onClick={() => handleOpenDetail(place)}>
                                                            View Details &rarr;
                                                        </button>
                                                    </div>
                                                </div>
